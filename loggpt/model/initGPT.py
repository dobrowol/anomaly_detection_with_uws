...
class InitGPT(nn.Module):
    def __init__(self, options, train_df, test_df, topk=None):
        ...
        self.sliding_window = options['sliding_window'] or options['segmentation']
    ...
    def _init_training(self):
        ...
        if self.init_logGPT:
            ...
            best_loss = math.inf
            for epoch in range(self.options['init_num_epochs']):
                self.model.train()
                epoch_loss = 0
                for batch in train_loader:
                    ...
                    outputs = self.model(inp, attention_mask=attention_mask, labels=inp)
                    loss = outputs.loss
                    loss.backward()
                    self.optim.step()
                ...
                mlflow.log_metric("dev_loss", epoch_loss/len(valid_loader))
